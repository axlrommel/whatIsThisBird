...
public class ImageComparer {
	...
	// private static String outputSystemPath = "/tmp/";
	...
	public BirdMatchingResults getMatchingResults(BufferedImage imageIn, Jedis jedis, String filename) throws Exception {
		...
		// System.out.println(inputStat.getTopRedsAsString() + " " +
		// inputStat.getTopGreensAsString() + " " +
		...
		for (int i = 0; i < list1.size(); i++) {
			...
			ImageStatistics fStat = new ImageStatistics(listR.toArray(new String[listR.size()]),
					listG.toArray(new String[listG.size()]), listB.toArray(new String[listB.size()]));
		...
		// can't do unionstore in the for loop to aggregate, we need to do it here,
		// let's get the max
		...
		jedis.zunionstore(totalResultsSet, z, appendString + "threecolor-corr", appendString + "high-corr",
				appendString + "low-corr", appendString + "red-corr", appendString + "green-corr", appendString + "blue-corr",
		...
		for (String s : qq) {
			...
			for (JedisCorrelationResults jf : filters) {
				if (jedis.zscore(jf.getFilterName(), s) != null) {
					...
					fResults.add(fr);
					System.out.println("\t" + s + " " + jf.getClientFilterName() + " " + jedis.zscore(jf.getFilterName(), s));
				}
			}
